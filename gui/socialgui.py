...
class Ui_MainWindow(object):
    def setupUi(self, So):
        ...
        self.pushButton.setStyleSheet(
            _fromUtf8("background-color:rgb(65, 65, 65); color:rgb(255, 255, 255);"))
        ...
        self.label_6.setStyleSheet(
            _fromUtf8("background-color:rgb(16, 170, 5); color: white;"))
    ...
    def retranslateUi(self, So):
        So.setWindowTitle(_translate(
            "So", "Social Network Tracking And Analysis", None))
        self.pushButton.setText(_translate("So", "Tracking", None))
        self.pushButton.clicked.connect(
            self.tracking)  # When the button is pressed

        self.pushButton_2.setText(_translate("So", "Analysis", None))
        self.pushButton_2.clicked.connect(
            self.analysis_tweet)  # When the button is pressed
        ...
        def receiveBuffer(tweets):
            ...
            for t in tweets:
                ...
                for hash in str:
                    ...
                    if hash != "":
                        ...
                        if exits is None:
                            ...
                            c.execute(
                                "INSERT OR IGNORE INTO HashtagTweet VALUES (?,?)", paramsHashagTweet)
                            c.execute(
                                "INSERT OR IGNORE INTO Hashtag  VALUES (?,?)", paramsHashtag)
                ...
                if userexist is None:
                    c.execute(
                        "INSERT INTO Tweet VALUES (?,?,?,?,?,?,?,?,?,?,?)", paramsTweet)
                ...
                c.execute("SELECT *FROM location where place = '%s'" % t.geo)
                locatuid = c.fetchone()
                paramsUser = (t.user_id, t.username,
                              locatuid, t.follow, t.follower)
                c.execute("SELECT * FROM user where username ='%s'" %
                          t.username)
                userexist = c.fetchone()
                if userexist is None:
                    c.execute(
                        "INSERT OR IGNORE INTO User VALUES(?,?,?,?,?)", paramsUser)
